...
public class KardexFacadeREST extends AbstractFacade<Kardex> {
    ...
    @Path("{id}")
    @Consumes({"application/xml", "application/json"})
    public void edit(@PathParam("id") Integer id, Kardex entity) {
        super.edit(entity);
    }
    ...
    public Kardex find(@PathParam("id") Integer id) {
    ...
    @Path("{from}/{to}")
    @Produces({"application/xml", "application/json"})
    public List<Kardex> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) {
